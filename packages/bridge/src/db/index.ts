...
export class NRC721Query {
  ...
  async *collectNonMinedTokens(): AsyncGenerator<NRC721.TokenWithFactoryScript> {
    ...
    const tokens = await this.knex<NRC721.Token.DBStruct>(
    ...
    const factories = await this.knex<NRC721.FactoryScript.DBStruct>(
      NRC721.FactoryScript.DB_TABLE_NAME
  ...
  async getTokenTipBlockNumber(): Promise<bigint> {
    const blockNumber = await this.knex<NRC721.Token.DBStruct>(
      NRC721.Token.DB_TABLE_NAME
    ).max("block_number");
  ...
  async getOneByOutPoint(
    outPoint: OutPoint
  ): Promise<NRC721.TokenWithFactoryScript | undefined> {
    const token = await this.knex<NRC721.Token.DBStruct>(
      NRC721.Token.DB_TABLE_NAME
    )
      .where({
    ...
    const factoryScript = await this.knex<NRC721.FactoryScript.DBStruct>(
      NRC721.FactoryScript.DB_TABLE_NAME
    )
      .where({
  ...
  async getOneByLayer1Token(
    ...
    const factory = await this.knex<NRC721.FactoryScript.DBStruct>(
      NRC721.FactoryScript.DB_TABLE_NAME
    )
      .where({
  ...
  private async saveFactoryScriptIfNotExists(
    factoryScript: NRC721.FactoryScript.DBStruct,
    now: Date
  ): Promise<bigint> {
    const one = await this.knex<NRC721.FactoryScript.DBStruct>(
      NRC721.FactoryScript.DB_TABLE_NAME
    )
      .where({
    ...
    const result = await this.knex<NRC721.FactoryScript.DBStruct>(
      NRC721.FactoryScript.DB_TABLE_NAME
    )
      .insert(factoryScript)
      .returning("id");
  ...
  async saveToken(tokenWithFactoryScript: NRC721.TokenWithFactoryScript) {
    ...
    const dbFactoryScript = NRC721.FactoryScript.toDB(
      tokenWithFactoryScript.factory_script
    );
    ...
    return await this.knex<NRC721.Token.DBStruct>(
      NRC721.Token.DB_TABLE_NAME
    ).insert(dbToken);
  }
  ...
  async saveIfNotExists(
    tokenWithFactoryScript: NRC721.TokenWithFactoryScript
  ): Promise<boolean> {
    const one = await this.getOneByOutPoint(
      tokenWithFactoryScript.token.out_point
    );
    if (one != null) {
      return false;
    }
  ...
  async updateToMined(layer2_token_id: bigint) {
    const now = new Date();
    await this.knex<NRC721.Token.DBStruct>(NRC721.Token.DB_TABLE_NAME)
      .where({ layer2_token_id: layer2_token_id.toString() })
      .update({ layer2_has_mined: true, updated_at: now });
  }
}
